...
export default function browser(state = initialState, action) {
    switch (action.type) {
        ...
        case Types.UPDATE_STACK_GO:
            if(action.data.updatetype == 1){
                state.stackGo.push(action.data.value)
            }
            else if (action.data.updatetype == 2){
               state.stackGo.pop()
            }
            return{...state}
        case Types.UPDATE_STACK_BACK:
            if(action.data.updatetype == 1){
                state.stackBack.push(action.data.value)
            }
            else if (action.data.updatetype == 2){
                state.stackBack.pop()
            }
